...
export default function MobileNav() {
  const { activeTab, setActiveTab, leads, dismissed, followUps, getColdLeads } = useStore();
  ...
  return (
    <nav style={{
      ...
      {ITEMS.map(item => {
        ...
        const badge =
          item.id === "leads"   && activeCount > 0 ? activeCount :
        ...
        return (
          <button key={item.id} onClick={() => setActiveTab(item.id)} style={{
            ...
            {item.id === "leads" && coldLeads.length > 0 && (
              <span style={{
                position: "absolute", top: 5, left: "50%", transform: "translateX(-2px)",
                background: "#f59e0b", color: "#fff", fontSize: 8, fontWeight: 700,
                borderRadius: 10, padding: "0 3px", lineHeight: "13px",
